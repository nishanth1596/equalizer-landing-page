...
const Button = ({ name, type, os }) => {
  const [isHovered, setIsHovered] = useState(false);

  const base =
    "flex min-w-[20.94rem] items-center justify-center gap-2 rounded-xl py-4 text-fs-6 font-fw-6 leading-lh-6 md:min-w-[16.25rem]";

  const styles = {
    dark: {
      className: base + " bg-slate text-white hover:bg-cyan hover:text-slate ",
      fillColor: "#fcfaf9 ",
    },
    light: {
      className: base + " bg-white text-slate hover:bg-yellow",
      fillColor: "#191826",
    },
  };
  ...
  return (
    ...
    >
      {os === "ios" ? (
        <Apple fillColor={styles[type].fillColor} isHovered={isHovered} />
      ) : (
        <Android fillColor={styles[type].fillColor} />
      )}
      {name}
    </button>
  );
...
function Apple({ fillColor, isHovered }) {
  return (
    <svg width="18" height="20" xmlns="http://www.w3.org/2000/svg">
      <path
        ...
        fill={isHovered ? "#191826" : fillColor}
      />
    </svg>
  );
}

function Android({ fillColor }) {
  return (
